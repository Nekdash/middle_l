#include "middle_list.h"
 ...
 void itc_pos_neg_analysis_lst(const vector <int> &lst){
    ...
    cout << "Сумма чисел: "<< itc_sumlst(pol) << "," << '\t'	<< '\t' << "Сумма чисел: "<< itc_sumlst(neg) <<"," << endl;
    cout << "Среднее значение: "<< itc_sumlst(pol) / pol.size()<< '\t'	<< '\t' << "Среднее значение: " << itc_sumlst(neg) / neg.size() << endl;
   cout << endl;
